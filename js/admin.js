...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function setAdminBudget(amount) {
        try {
            const adminUid = auth.currentUser.uid;
            const timestamp = Date.now();
            
            // Update total budget
            await database.ref('budget').update({
                total: amount,
            ...
            await database.ref('budget_history').push({
                amount: amount,
                timestamp: timestamp,
                adminUid: adminUid,
                type: 'set'
    ...
    async function allocateBudgetToSubAdmin(subAdminUid, amount) {
        try {
            ...
            // Check if enough budget is available
            if (totalBudget - totalAllocated < amount) {
                showMessage('Insufficient budget available', 'error');
                return false;
            }
            ...
            console.error('Error allocating budget:', error);
            showMessage('Failed to allocate budget', 'error');
            return false;
        }
    }
    ...
    async function reverseBudgetFromSubAdmin(subAdminUid, amount) {
        try {
            ...
            // Get sub-admin's current budget info
            const subAdminRef = database.ref(`users/${subAdminUid}`);
            const subAdminSnapshot = await subAdminRef.once('value');
            const subAdminData = subAdminSnapshot.val() || {};
            ...
            const currentAllocated = subAdminData.allocatedBudget || 0;
            const currentAvailable = subAdminData.availableBalance || 0;
            const currentUsed = subAdminData.usedBudget || 0;

            // Check if amount can be reversed
            if (currentAvailable < amount) {
                showMessage('Cannot reverse more than available balance', 'error');
                return false;
            }
            ...
            showMessage('Failed to reverse budget', 'error');
            return false;
        }
    }

    // Add event listeners for budget management
    if (setBudgetForm) {
        setBudgetForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            const amount = parseFloat(this.querySelector('#budgetAmount').value);
            if (amount > 0) {
                await setAdminBudget(amount);
            } else {
                showMessage('Please enter a valid amount', 'error');
            }
        });
    ...
    // Add event listeners for sub-admin budget allocation
    ...
    if (allocateBudgetForm) {
        allocateBudgetForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            const subAdminUid = this.querySelector('#subAdminSelect').value;
            const amount = parseFloat(this.querySelector('#allocationAmount').value);
            
            if (subAdminUid && amount > 0) {
                await allocateBudgetToSubAdmin(subAdminUid, amount);
                this.reset();
            } else {
                showMessage('Please select a sub-admin and enter a valid amount', 'error');
            }
        });
    }

    // Add event listeners for budget reversal
    const reverseBudgetForm = document.getElementById('reverseBudgetForm');
    if (reverseBudgetForm) {
        reverseBudgetForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            const subAdminUid = this.querySelector('#reverseSubAdminSelect').value;
            const amount = parseFloat(this.querySelector('#reversalAmount').value);
