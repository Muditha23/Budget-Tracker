...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const adminBudget = document.getElementById('adminBudget');
    const lastBudgetUpdate = document.getElementById('lastBudgetUpdate');
    ...
    async function updateAdminBudget(amount) {
        try {
            ...
            // Update admin's budget
            const currentBudget = adminData.budget || 0;
            const newBudget = currentBudget + amount;
            
            await adminRef.update({
                budget: newBudget,
                lastBudgetUpdate: firebase.database.ServerValue.TIMESTAMP
            });
            
            showMessage(`Budget updated successfully. New balance: ${formatCurrency(newBudget)}`, 'success');
            await loadDashboardData();
        } catch (error) {
    ...
    async function allocateBudgetToSubAdmin(subAdminUid, amount) {
        try {
            const adminUid = firebase.auth().currentUser.uid;
            const adminRef = database.ref(`users/${adminUid}`);
            ...
            const [adminSnapshot, subAdminSnapshot] = await Promise.all([
                adminRef.once('value'),
                subAdminRef.once('value')
            ]);
            ...
            const adminBudget = adminData.budget || 0;
            if (adminBudget < amount) {
                throw new Error('Insufficient admin balance');
            }
            
            // Update balances
            const updates = {};
            updates[`users/${adminUid}/budget`] = adminBudget - amount;
            updates[`users/${subAdminUid}/allocatedBudget`] = (subAdminData.allocatedBudget || 0) + amount;
            updates[`users/${subAdminUid}/availableBalance`] = (subAdminData.availableBalance || 0) + amount;
            
            // Record allocation
            const allocationRef = database.ref('budget_allocations').push();
            updates[`budget_allocations/${subAdminUid}/${allocationRef.key}`] = {
                amount: amount,
                type: 'allocation',
                timestamp: firebase.database.ServerValue.TIMESTAMP,
                adminUid: adminUid
            ...
            console.error('Error allocating budget:', error);
            showMessage(error.message || 'Failed to allocate budget', 'error');
        }
    }

    async function reverseBudgetFromSubAdmin(subAdminUid, amount) {
        try {
            const adminUid = firebase.auth().currentUser.uid;
            const adminRef = database.ref(`users/${adminUid}`);
            const subAdminRef = database.ref(`users/${subAdminUid}`);
            ...
            // Check if sub-admin has sufficient available balance
            const subAdminAvailable = subAdminData.availableBalance || 0;
            if (subAdminAvailable < amount) {
                throw new Error('Insufficient sub-admin available balance');
            }
            
            // Update balances
            const updates = {};
            updates[`users/${adminUid}/budget`] = (adminData.budget || 0) + amount;
            updates[`users/${subAdminUid}/allocatedBudget`] = (subAdminData.allocatedBudget || 0) - amount;
            updates[`users/${subAdminUid}/availableBalance`] = subAdminAvailable - amount;
            
            // Record reversal
            const reversalRef = database.ref('budget_allocations').push();
            updates[`budget_allocations/${subAdminUid}/${reversalRef.key}`] = {
                amount: amount,
                type: 'reversal',
                timestamp: firebase.database.ServerValue.TIMESTAMP,
                adminUid: adminUid
            ...
            showMessage(error.message || 'Failed to reverse budget', 'error');
        }
    }

    async function receiveReturnedBudget(subAdminUid, amount) {
        try {
            const adminUid = firebase.auth().currentUser.uid;
            const adminRef = database.ref(`users/${adminUid}`);
            const subAdminRef = database.ref(`users/${subAdminUid}`);
            ...
            updates[`users/${subAdminUid}/allocatedBudget`] = (subAdminData.allocatedBudget || 0) - amount;
            updates[`users/${subAdminUid}/availableBalance`] = (subAdminData.availableBalance || 0) - amount;
            updates[`users/${subAdminUid}/returnedBudget`] = (subAdminData.returnedBudget || 0) + amount;
            
            // Record return
            const returnRef = database.ref('budget_returns').push();
            updates[`budget_returns/${subAdminUid}/${returnRef.key}`] = {
                amount: amount,
                timestamp: firebase.database.ServerValue.TIMESTAMP,
                adminUid: adminUid
            };
            ...
            showMessage(`Successfully received ${formatCurrency(amount)} from sub-admin`, 'success');
            await loadDashboardData();
        } catch (error) {
            console.error('Error receiving returned budget:', error);
            showMessage('Failed to receive returned budget', 'error');
        }
    }

    // Budget Management Event Listeners
    if (updateBudgetForm) {
        updateBudgetForm.addEventListener('submit', async function(e) {
            ...
            await updateAdminBudget(amount);
    ...
    if (allocateBudgetForm) {
        allocateBudgetForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            const subAdminUid = subAdminSelect.value;
            const amount = parseFloat(allocateAmount.value);
            
            if (!subAdminUid) {
                showMessage('Please select a sub-admin', 'error');
                return;
            }
            ...
            await allocateBudgetToSubAdmin(subAdminUid, amount);
    ...
    if (reverseBtn) {
        reverseBtn.addEventListener('click', async function() {
            ...
            await reverseBudgetFromSubAdmin(subAdminUid, amount);
            allocateAmount.value = '';
        });
    }

    // Budget Management Display Functions
    async function updateBudgetDisplay() {
        try {
            const adminUid = firebase.auth().currentUser.uid;
            const adminSnapshot = await database.ref(`users/${adminUid}`).once('value');
            const adminData = adminSnapshot.val() || {};
            
            if (adminBudget) {
                adminBudget.textContent = formatCurrency(adminData.budget || 0);
            }
            if (lastBudgetUpdate) {
                lastBudgetUpdate.textContent = adminData.lastBudgetUpdate ? 
                    formatDate(adminData.lastBudgetUpdate) : 'Never';
            }
                
            await updateBudgetHistory();
        } catch (error) {
            console.error('Error updating budget display:', error);
        }
    }

    async function updateBudgetHistory() {
        try {
            const adminUid = firebase.auth().currentUser.uid;
            const budgetHistory = document.getElementById('budgetHistory');
            if (!budgetHistory) return;
            
            // Get all budget-related transactions
            ...
            // Process allocations
            allocationsSnapshot.forEach(userSnapshot => {
                userSnapshot.forEach(allocationSnapshot => {
                    const allocation = allocationSnapshot.val();
                    if (allocation.adminUid === adminUid) {
                        transactions.push({
                            ...allocation,
                            subAdminUid: userSnapshot.key,
                            id: allocationSnapshot.key,
            ...
            // Process returns
            returnsSnapshot.forEach(userSnapshot => {
                userSnapshot.forEach(returnSnapshot => {
                    const returnData = returnSnapshot.val();
                    if (returnData.adminUid === adminUid) {
                        transactions.push({
                            ...returnData,
                            subAdminUid: userSnapshot.key,
                            id: returnSnapshot.key,
            ...
            const subAdminsSnapshot = await database.ref('users')
                .orderByChild('role')
                .equalTo('subadmin')
                .once('value');
            ...
            if (transactions.length === 0) {
                budgetHistory.innerHTML = '<p class="text-gray-500">No budget transactions yet</p>';
                return;
            }
            
            const historyHTML = transactions.map(transaction => {
                const subAdmin = subAdmins[transaction.subAdminUid] || {};
                const typeClass = transaction.transactionType === 'allocation' ? 'text-green-600' :
                                transaction.transactionType === 'reversal' ? 'text-yellow-600' :
                                'text-blue-600';
                const typeText = transaction.transactionType === 'allocation' ? 'Allocated to' :
                               transaction.transactionType === 'reversal' ? 'Reversed from' :
                               'Returned by';
                
                return `
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <div>
                            <p class="font-medium text-gray-800">${typeText} ${subAdmin.email || 'Unknown Sub-Admin'}</p>
                            <p class="text-sm text-gray-600">${formatDate(transaction.timestamp)}</p>
                        </div>
                        <p class="font-semibold ${typeClass}">${formatCurrency(transaction.amount)}</p>
                    </div>
                `;
            }).join('');
            
            budgetHistory.innerHTML = historyHTML;
            
        } catch (error) {
            console.error('Error updating budget history:', error);
            budgetHistory.innerHTML = '<p class="text-red-500">Error loading budget history</p>';
        }
    }

    async function updateSubAdminSelect() {
        try {
            const subAdminSelect = document.getElementById('subAdminSelect');
            ...
            const options = Object.entries(subAdmins).map(([uid, data]) => `
                <option value="${uid}">${data.email}</option>
            `);
            
            subAdminSelect.innerHTML = '<option value="">Select Sub-Admin</option>' + options.join('');
        } catch (error) {
            console.error('Error updating sub-admin select:', error);
            showMessage('Error loading sub-admins', 'error');
        }
    }
});
