...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function returnBudgetToAdmin(amount) {
        try {
            const subAdminUid = firebase.auth().currentUser.uid;
            const subAdminRef = database.ref(`users/${subAdminUid}`);
            
            // Get current sub-admin data
            ...
            const availableBalance = subAdminData.availableBalance || 0;
            if (availableBalance < amount) {
                throw new Error('Insufficient available balance');
            }
            
            // Get admin data
            const adminUid = subAdminData.adminUid;
            if (!adminUid) {
            ...
            updates[`users/${subAdminUid}/availableBalance`] = availableBalance - amount;
            ...
            showMessage(`Successfully returned ${formatCurrency(amount)} to admin`, 'success');
            await initializeBudgetInfo();
        } catch (error) {
            console.error('Error returning budget:', error);
            showMessage(error.message || 'Failed to return budget', 'error');
        }
    }

    // Add event listener for budget return form
